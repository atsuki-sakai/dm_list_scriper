...
function hiraganaToRomaji(hiragana: string): string[] {
    ...
    function convertRecursive(remaining: string, current: string): void {
 ...
 * @returns マッチ度（0-1）
 */
export function calculateRomajiSimilarity(target: string, pattern: string): number {
    const targetLower = target.toLowerCase();
    const patternLower = pattern.toLowerCase();
    
    // 完全一致
    if (targetLower === patternLower) return 1.0;
 ...
 * @returns 検索キーワードの配列
 */
export function generateInstagramSearchKeywords(salonName: string): string[] {
