...
import { SubArea, DetailArea, SalonDetails } from '../types/index';
import { sleep, resolveUrl, extractQueryParam, removeDuplicates, normalizeText } from '../utils/index';
import { DELAY_MS, SELECTORS } from '../constants/index';
 ...
 * @returns 最後のサロンのURL
 */
export async function getLastSalonUrl(listPageUrl: string): Promise<string | undefined> {
    try {
        ...
        const finalAnchor = lastSalonLi.find(SELECTORS.SALON_LINKS).first();
        const finalHref = finalAnchor.attr('href');

        if (finalHref) {
            return resolveUrl(finalHref, listPageUrl);
        }
 ...
 */
export async function findSalonByName(listPageUrl: string, targetName: string): Promise<string | undefined> {
    try {
        await sleep(DELAY_MS);
        const { data } = await axios.get(listPageUrl);
        const $ = cheerio.load(data);
        ...
        salonLis.each((_, el) => {
            ...
            if (salonName.includes(targetName)) {
                ...
                if (href) {
                    foundUrl = resolveUrl(href, listPageUrl);
